...
class Spinner {
  ...
  constructor(name, config) {
    ...
    this.element = null;
    this.backdrop = null;
    this.id = this.config.id || `spinner-${this.name}-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
  }
...
create() {
  ...
  if ((this.config.position === 'fixed' || this.config.position === 'absolute') && this.config.showBackdrop) {
