		...
		const handleMouseUp = () => {
			if (isDragging && onChange && value !== undefined) {
				...
				if (type === 'single') {
					...
					valueChanged = startValue !== initialDragStartValue || endValue !== initialDragEndValue;
				}
...
<div use:handleDrag class={['range', cls, `range--${color}`].join(' ')}>
	<div class="range--track" bind:this={track}>
		<div
	...
	<div
		class="range--handle range--handle-start {isDragging && activeDot === 'start'
			? 'range--handle-active'
			: ''}"
	...
	{#if type === 'range'}
		<div
			class="range--handle range--handle-end {isDragging && activeDot === 'end'
				? 'range--handle-active'
				: ''}"
