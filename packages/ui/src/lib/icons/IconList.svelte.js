/**
 * This file dynamically scans the images directory for SVG files
 * and exports a function to get all available icon names.
 */

/**
 * Get all SVG files from the images directory
 * @returns {Promise<string[]>} Array of icon names (without .svg extension)
 */
export async function getIconNames() {
  try {
    // Use Vite's import.meta.glob to get all SVG files
    const svgFiles = import.meta.glob('./images/*.svg', { eager: false });
    
    // Extract icon names from file paths
    const iconNames = Object.keys(svgFiles)
      .map(path => {
        // Extract filename without extension from path
        // ./images/icon-name.svg -> icon-name
